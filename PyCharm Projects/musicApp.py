...
def play_one(num_date, one_song):
    chart = billboard.ChartData('hot-100', date=num_date)
    rand_song = chart[one_song - 1]
    print(rand_song)
...
def just_one(num_date):
    ...
    answer = input("Would you like to play this song on Youtube? Y or N:")
    if answer.lower() == "y":
        search_bar = str(song)
        query = urllib.parse.quote(search_bar)
        searches = list()
...
if __name__ == '__main__':
    ...
    bar_graph = input("Do you want a bar graph that shows the"
                  "number of weeks an artist was on the charts? Y or N:")
